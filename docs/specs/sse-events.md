...
- **变更记录**：v1.1 澄清降级语义——第 3 次连续失败进入轮询降级，**但 SSE 重连不放弃**：继续按 5/10 秒退避重试，任一次重连成功即自动升级并停止轮询（消解 v1.0 中"退避 1/2/5/10 封顶"与"3 次失败降级"的互斥表述）。
...
- 重连时序（客户端状态机，v1.1）：第 1/2 次失败按 1s/2s 退避重连；第 3 次连续失败**进入轮询降级**（前台 3s/后台 15s），同时以 5s→10s（封顶）退避继续重试 SSE；任一次重连成功即升级回 connected 并停轮询；`online` 事件可提前触发升级。
